...
async def cmd_backtest(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Выберите стратегию для бэктеста:", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def enter_ticker(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("За какой период прогнать бэктест?", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def choose_period_and_run(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        result = run_backtest(df, strategy, limits, lot_size=instrument.lot)
    ...
    png = render_equity_curve_png(result)
    ...
    await callback.message.answer_photo(BufferedInputFile(png, filename="equity_curve.png"), caption=summary)
